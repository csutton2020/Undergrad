...
public class Hand {
	...
	static Hand EvaluateHand(Hand h) throws HandException {
		...
		ArrayList<Hand> sortedExplodedHands = new ArrayList<Hand>();
		
		sortedExplodedHands = sortEachExplodeHands(ExplodedHands);

		for (Hand hEval : sortedExplodedHands) {
	...
	private static ArrayList<Hand> sortEachExplodeHands(ArrayList<Hand> ALH){
		for(Hand h: ALH){
			Collections.sort(h.getCardsInHand());
		}
		return ALH;
